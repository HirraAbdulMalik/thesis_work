...
class KGEModel(nn.Module):
    ...
    def forward(self, sample, mode='single'):
        ...
        if mode == 'single' and self.model_name != 'MDE':
           ...
           # print('r - ', r.size())
            ...
            head = [h, h2, h3, h4, h5,h6,h7,h8]
            #print('head here 691 - ', head[0].size())
            relation = [r, r2, r3, r4,r5,r6,r7,r8]
            #print('relation ere 691 - ', relation[0].size())
            tail = [t, t2, t3, t4, t5,t6,t7,t8]
            #print('tail here 691 -', tail[0].size())
        else:
    ...
    def MDE(self, h, r, t, mode):
        ...
        if mode == 'head-batch':
            ...
            #print(mode)
            #print('here h is the size \n')
            #print(h[0].size())
            #print('here is r size \n')
            #print(r[0].size())
            #print('here is t size \n')
            #print(t[0].size())
            a = (h[0] + r[0]) - t[0]
            b = (h[1] + t[1]) - r[1]
            c = (t[2] + r[2]) - h[2]
        ...
        if args.countries:
            ...
            with torch.no_grad():
                for test_dataset in test_dataset_list:
                    for positive_sample, negative_sample, filter_bias, neg_triple, mode in test_dataset:
                        #print('here2')
                        if args.cuda:
                            positive_sample = positive_sample.cuda()
                            negative_sample = negative_sample.cuda()
                        ...
                        if mode == 'head-batch':
                            ...
                            raise ValueError('mode %s not supported' % mode)
                        #print('positive_arg', positive_arg)
                        for i in range(batch_size):
                           # print('i', i)
                            # Notice that argsort is not ranking                            
                            ranking = (argsort[i, :] == positive_arg[i]).nonzero()
                            #print('ranking after : ',ranking[0])
                        ...
                        #for auc
                        #pos_score
                        score_pred_pos = model(positive_sample)
                        #print('pos_sample', positive_sample)
                        #print('pos_score', score_pred_pos)
                        p = [1 for i in range(len(positive_sample))]
                        ans.extend(list(p))
                        res_scores.extend(list(score_pred_pos))
                        #print(ans)
                        #print(res_scores)
                        #print(len(ans))
                        ...
                        #break
                        #neg_score 
                        score_pred_neg = model(neg_triple)
                        #print('neg_sample', neg_triple)
                        #print('neg_score', score_pred_neg)
                        n = [0 for i in range(len(neg_triple))]
                        ans.extend(list(n))
                        res_scores.extend(list(score_pred_neg))
                        #print(len(ans))
                        #print(len(res_scores))
                        ...
                        step += 1
            res_scores = [t.cpu().numpy() for t in res_scores]
            #print(len(res_scores))
            #print(len(ans))
            #print('score', np.asarray(res_scores))
            #print('ans', ans) 
            
            #auc = roc_auc_score(ans, res_scores)
            #print('\nAUC - : ', auc) 
            auc_pr = average_precision_score(ans, res_scores)  
            print('\nAUC_PR - :',auc_pr)                                                                                       

            #logs.append({'auc_', auc, 
            #             'auc_pr', auc_pr})
